___TERMS_OF_SERVICE___

By creating or modifying this file you agree to Google Tag Manager's Community
Template Gallery Developer Terms of Service available at
https://developers.google.com/tag-manager/gallery-tos (or such other URL as
Google may provide), as modified from time to time.


___INFO___

{
  "type": "TAG",
  "id": "cvt_temp_public_id",
  "version": 1,
  "securityGroups": [],
  "displayName": "Insider Cart Products",
  "brand": {
    "id": "brand_dummy",
    "displayName": "",
    "thumbnail": "data:image/png;base64,\u003d\u003d"
  },
  "description": "This template collects cart products from dataLayer. You should specify the variable. It interprets dots as nested values. For example, \"a.b.c\" as three nested levels: {a: {b: {c: \"value\"}}}.",
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[
  {
    "type": "TEXT",
    "name": "cartProducts",
    "displayName": "Parent Variable",
    "simpleValueType": true,
    "help": "You should write the array of object variable name which keeps all products. For example, interpret \"a.b.c\" as three nested levels: {a: {b: {c: \"value\"}}}.",
    "valueValidators": [
      {
        "type": "NON_EMPTY",
        "errorMessage": "This input required.",
        "enablingConditions": []
      }
    ],
    "defaultValue": "cartProducts"
  },
  {
    "type": "TEXT",
    "name": "productId",
    "displayName": "Product Id Variable Name",
    "simpleValueType": true,
    "help": "You should write the variable name which keeps product id. It interpret dots as nested values. For example, interpret \"a.b.c\" as three nested levels: {a: {b: {c: \"value\"}}}.",
    "valueValidators": [
      {
        "type": "NON_EMPTY",
        "errorMessage": "This input required."
      }
    ],
    "defaultValue": "id"
  },
  {
    "type": "TEXT",
    "name": "productName",
    "displayName": "Product Name Variable Name",
    "simpleValueType": true,
    "help": "You should write the variable name which keeps product name. For example, interpret \"a.b.c\" as three nested levels: {a: {b: {c: \"value\"}}}.",
    "valueValidators": [
      {
        "type": "NON_EMPTY",
        "errorMessage": "This input required."
      }
    ],
    "defaultValue": "name"
  },
  {
    "type": "TEXT",
    "name": "productPrice",
    "displayName": "Product Price Variable Name",
    "simpleValueType": true,
    "help": "You should write the variable name which keeps product price.For example, interpret \"a.b.c\" as three nested levels: {a: {b: {c: \"value\"}}}.",
    "valueValidators": [
      {
        "type": "NON_EMPTY",
        "errorMessage": "This input required."
      }
    ],
    "defaultValue": "price"
  },
  {
    "type": "TEXT",
    "name": "productOriginalPrice",
    "displayName": "Product Original Price Variable Name",
    "simpleValueType": true,
    "help": "You should write the variable name which keeps product original price. For example, interpret \"a.b.c\" as three nested levels: {a: {b: {c: \"value\"}}}.",
    "valueValidators": [
      {
        "type": "NON_EMPTY",
        "errorMessage": "This input required."
      }
    ],
    "defaultValue": "originalPrice"
  },
  {
    "type": "TEXT",
    "name": "productUrl",
    "displayName": "Product Url Variable Name",
    "simpleValueType": true,
    "help": "You should write the variable name which keeps product url. For example, interpret \"a.b.c\" as three nested levels: {a: {b: {c: \"value\"}}}.",
    "valueValidators": [
      {
        "type": "NON_EMPTY",
        "errorMessage": "This input required."
      }
    ],
    "defaultValue": "url"
  },
  {
    "type": "TEXT",
    "name": "productImage",
    "displayName": "Product Image Variable Name",
    "simpleValueType": true,
    "help": "You should write the variable name which keeps product image. For example, interpret \"a.b.c\" as three nested levels: {a: {b: {c: \"value\"}}}.",
    "valueValidators": [
      {
        "type": "NON_EMPTY",
        "errorMessage": "This input required."
      }
    ],
    "defaultValue": "image"
  },
  {
    "type": "TEXT",
    "name": "productQuantity",
    "displayName": "Product Quantity Variable Name",
    "simpleValueType": true,
    "help": "You should write the variable name which keeps product quantity. For example, interpret \"a.b.c\" as three nested levels: {a: {b: {c: \"value\"}}}.",
    "valueValidators": [
      {
        "type": "NON_EMPTY",
        "errorMessage": "This input required."
      }
    ],
    "defaultValue": "quantity"
  },
  {
    "type": "TEXT",
    "name": "productCategories",
    "displayName": "Product Categories Variable Name",
    "simpleValueType": true,
    "help": "You should write the variable name which keeps product categories. For example, interpret \"a.b.c\" as three nested levels: {a: {b: {c: \"value\"}}}.",
    "valueValidators": [
      {
        "type": "NON_EMPTY",
        "errorMessage": "This input required.",
        "enablingConditions": []
      }
    ],
    "defaultValue": "categories"
  }
]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

const copyFromWindow = require('copyFromWindow');
const setInWindow = require('setInWindow');

const insiderObject = copyFromWindow('insider_object') || {};
const dataLayer = copyFromWindow('dataLayer');

var getNestedData = function (parentData, dataFieldPath) {
    if (typeof dataFieldPath === 'string') {
        dataFieldPath = dataFieldPath.split('.');
    }

    if (typeof parentData !== 'undefined') {
        dataFieldPath.forEach(function (field) {
            parentData = parentData[field];
        });
    }

    return parentData;
};

var getData = function (data) {
    var dataFieldPath = data.split('.');
    var parentDataName = dataFieldPath[0];
    var parentData;

    dataFieldPath.shift();

    dataLayer.some(function (windowData) {
        if (typeof windowData[parentDataName] !== 'undefined' && 
            typeof windowData[parentDataName][0] !== 'undefined') {
            parentData = windowData[parentDataName];

            return true;
        }
    });

    return getNestedData(parentData, dataFieldPath);
};

insiderObject.basket = insiderObject.basket || {};
insiderObject.basket.line_items = insiderObject.basket.line_items || [];

(getData(data.cartProducts) || []).forEach(function(product) { 
    insiderObject.basket.line_items.push({
        product: {
            id: getNestedData(product, data.productId) || '',
            name: getNestedData(product, data.productName) || '',
            taxonomy: getNestedData(product, data.productCategories) || [],
            unit_sale_price: getNestedData(product, data.productPrice) || '0',
            unit_price: getNestedData(product, data.productOriginalPrice) || '0',
            url: getNestedData(product, data.productUrl) || '',
            product_image_url: getNestedData(product, data.productImage) || ''
        },
        quantity: getNestedData(product, data.productQuantity) || '0'
    });
});

setInWindow('insider_object', insiderObject, true);

data.gtmOnSuccess();


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "access_globals",
        "versionId": "1"
      },
      "param": [
        {
          "key": "keys",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "dataLayer"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              },
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "insider_object"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  }
]


___TESTS___

scenarios: []


___NOTES___

Created on 7/28/2020, 4:17:09 PM


